...
7. The inventory is one row per visible semantic object. Never combine separate labels into one large text box, and never use 1x1 or off-canvas text as a placeholder. For every arrow, connector, decision branch, icon, repository/document mark, brace, and panel, record an expected object id and rebuild it as a separate selectable object. Count expected versus rebuilt objects in `visual_audit`; any deficit is a page failure.
