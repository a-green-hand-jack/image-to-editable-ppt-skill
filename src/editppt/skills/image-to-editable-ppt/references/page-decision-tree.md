...
- Put as many icons and foreground visual objects as practical onto one sparse asset sheet. For dense pages, repeated marks, or overlapping subjects, use small serial sheets with explicit source-instance IDs and coordinates; a single large sheet is unsuitable when the model cannot preserve count, order, and independent contours. Create multiple sheets when one sheet cannot fit or reliably separate all required objects. A failed large sheet is not reusable evidence of successful separation: inspect its missing/merged objects, change the batching or edit target, and reconcile each new sheet independently.
...
- The preview renderer must use a scalable installed font. If it falls back to Pillow's fixed-size bitmap font, the comparison is invalid: repair the runtime font fallback before judging text size. Compare rendered glyph height and line breaks, not the requested point value alone; a measured value copied from an OCR box is not acceptance evidence when the rendered text visibly differs from the source.
...
Whoever rebuilds the page checks it once against this list; after a repair, recheck the affected objects and their layout dependencies. Do not rerun unchanged visual checks. Deterministic validation is necessary but not sufficient, and the parent performs a source-versus-render handoff review without repeating unchanged page-local repair checks. Use render-validation.md for actual PPTX rendering. Record the evidence in structured manifest fields and `validation.json`. (Deck-level structural QA at finalize time is in `SKILL.md` section "4. Recover and finalize".)
