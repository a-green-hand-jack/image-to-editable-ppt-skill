...
> Responsibility: give a page owner actionable reconstruction and handoff instructions. Maintain placeholders and reference routing with the packaged skill; do not duplicate detailed contracts or override their exceptions.
...
- Start with page_request.json and page-decision-tree.md's introduction, "Common Failure Mode: False Progress", and "Pre-Decision: Page Inventory", including "Source layout anchors". Inspect the source and build the inventory and layout relationships, then read sections 1 and 2 for object-source decisions.
...
1. Preflight image tools and upload authorization before foreground work. Use image editing for complex artwork; if it is unavailable, simple flat pictograms may use the explicitly recorded `native-approximate` fallback in page-decision-tree.md section 2.4. Never use that fallback for logos, photos, textures, or identity-bearing artwork.
...
7. The inventory is one structured row per visible source object. Every row needs a unique `id`, `role`, `object_type`, `source_box_px`, and `representation_ids`. Never combine separate labels into one large text box, and never use 1x1 or off-canvas text as a placeholder. For every arrow, connector, decision branch, icon, repository/document mark, brace, panel, person, robot, plant, device, sticker, or illustration, record a source object id and rebuild it as a separate selectable object. Complex semantic foreground objects must use image assets; simple flat pictograms may use `source_type: native-approximate` only with the documented warning and native shape representation. Set `visual_audit.source_visual_object_ids` to the exact inventory id set and derive `expected_semantic_visuals` from the inventory.
8. Every visible text box must include `font_size_source: "measured"` and a source-pixel `box_px` that fits the actual label with modest padding, not an unrelated card or panel. Badge-centered text is the explicit exception: reuse the base box with `align: "center"` and `valign: "middle"` under section 3.6. Do not let `fit_text` silently shrink a text level below the measured source size; correct geometry and wrapping while preserving the source label's grouping.
...
For formula work, apply section 3.2's engine/converter preflight and recovery. A broken PATH wrapper is not proof that host TeX is unavailable; use a discovered working executable via `--engine`. Missing formulas or styled-text substitutes are unresolved objects, even when their surrounding cards exist and structural counts otherwise pass.
...
2. Reconstruct native elements under the applicable section 3 rules and write manifest.json using its field contract. Resolve positions from the source layout anchors; retain compliant separated assets when correcting placement.
3. Run `editppt page build {{PAGE_DIR}}`, then `editppt page contact-sheet {{PAGE_DIR}}`.
4. Render page.pptx using render-validation.md and compare that actual render against source.png: overall layout first, group relationships next, then text and asset details. Record commands, renderer, paths, affected object IDs, and differences in visual_audit.review_notes; keep unresolved displacement in visual_audit.misaligned_object_ids. Perform the Final Self-Check and run `editppt page validate {{PAGE_DIR}}`. Fix page-local issues yourself; after a change rebuild and re-render the affected outputs. Once the current outputs pass, return without another unchanged build/QA cycle. If rendering is unavailable or fails, report that boundary; do not claim visual acceptance from preview.png alone.
