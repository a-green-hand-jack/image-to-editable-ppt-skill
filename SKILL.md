...
> **Project configuration (house).** The `editppt` CLI is installed with
> `uv tool install --editable <skill-root>/cli`; its image backend points at the
> Apex OpenAI-compatible endpoint (model `gpt-image-2.5-flare`). Run the page
> reconstructor with the Apex **`gpt-6-astra`** vision model, and size every
> text box from the measured capital glyph height (page-worker hard rule 6).
> Do NOT rely on the PaddleOCR text-hints backend: on this Ubuntu host it is
> slow and flaky, and this workflow does not need it. This is the supported
> configuration.
...
- Every page must also contain an explicit, source-grounded `visual_audit` record. A structurally valid PPTX is not visual acceptance: the audit enumerates the exact source visual inventory IDs, expected and rebuilt text boxes, arrows/connectors, semantic icons, and panel/container objects, and records any missing or misaligned object. `expected_semantic_visuals` is derived from the inventory and cannot be self-declared as zero when foreground icons or illustrations exist. `editppt run record` rejects missing audit evidence, unmeasured visible text, placeholder text boxes, missing source-object coordinates, and foreground visuals without image assets/provenance.
...
- Each page is self-checked once by the page reconstructor; the evidence is written into structured fields in `manifest.json` and into `validation.json`. The parent agent must independently inspect `split_assets_contact.png` or `preview.png` against `source.png` and reject any missing arrow, icon, connector, or displaced text before finalization.
