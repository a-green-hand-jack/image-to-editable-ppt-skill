...
- Every page must also contain an explicit `visual_audit` record. A structurally valid PPTX is not visual acceptance: the audit enumerates expected and rebuilt text boxes, arrows/connectors, semantic icons, and panel/container objects, and records any missing or misaligned object. `editppt run record` rejects missing audit evidence, unmeasured visible text, and placeholder text boxes.
